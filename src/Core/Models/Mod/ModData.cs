...
public partial class ModData : ReactiveObject, IModuleShortDesc
{
	...
	[Reactive] public partial string? FileName { get; private set; }
	...
	[ObservableAsProperty] public partial bool CanAddToLoadOrder { get; }
	...
	public ModData(string uuid)
	{
		...
		_hasDescriptionHelper = this.WhenAnyValue(x => x.Description).Select(Validators.IsValid).ToUIProperty(this, x => x.HasDescription);

		_hasAuthorHelper = this.WhenAnyValue(x => x.AuthorDisplayName).Select(Validators.IsValid).ToUIPropertyImmediate(this, x => x.HasAuthor);

		_canAddToLoadOrderHelper = this.WhenAnyValue(x => x.ModType, x => x.IsHidden, x => x.IsForceLoaded, x => x.IsForceLoadedMergedMod, x => x.ForceAllowInLoadOrder)
		...
		_nexusImageVisibilityHelper = this.WhenAnyValue(x => x.NexusModsData.PictureUrl).Select(Validators.IsValid).ToUIProperty(this, x => x.NexusImageVisibility);
		_nexusModsInformationVisibilityHelper = this.WhenAnyValue(x => x.NexusModsData.IsUpdated).ToUIProperty(this, x => x.NexusModsInformationVisibility);

		_nexusModsCreatedDateHelper = this.WhenAnyValue(x => x.NexusModsData.CreatedTimestamp).SkipWhile(x => x <= 0).Select(DateUtils.UnixTimeStampToDateTime).ToUIProperty(this, x => x.NexusModsCreatedDate);
		_nexusModsUpdatedDateHelper = this.WhenAnyValue(x => x.NexusModsData.UpdatedTimestamp).SkipWhile(x => x <= 0).Select(DateUtils.UnixTimeStampToDateTime).ToUIProperty(this, x => x.NexusModsUpdatedDate);

		_nexusModsTooltipInfoHelper = this.WhenAnyValue(x => x.NexusModsCreatedDate, x => x.NexusModsUpdatedDate, x => x.NexusModsData.EndorsementCount)
			.Select(x => NexusModsInfoToTooltip(x.Item1, x.Item2, x.Item3)).ToUIProperty(this, x => x.NexusModsTooltipInfo);

		_canForceAllowInLoadOrderHelper = this.WhenAnyValue(x => x.IsForceLoaded, x => x.HasMetadata, x => x.IsForceLoadedMergedMod)
			.Select(b => b.Item1 && b.Item2 && !b.Item3)
			.ToUIProperty(this, x => x.CanForceAllowInLoadOrder);

		_hasGitHubLinkHelper = this.WhenAnyValue(x => x.GitHubEnabled, x => x.GitHubData.IsEnabled, (b1, b2) => b1 && b2)
			.ToUIProperty(this, x => x.HasGitHubLink);

		_hasNexusModsLinkHelper = this.WhenAnyValue(x => x.NexusModsEnabled, x => x.NexusModsData.ModId, (b, id) => b && id >= DivinityApp.NEXUSMODS_MOD_ID_START)
			.ToUIProperty(this, x => x.HasNexusModsLink);

		_hasModioLinkHelper = this.WhenAnyValue(x => x.ModioEnabled, x => x.ModioData.ExternalLink, CanOpenModioBoolCheck)
		...
		_hasInvalidUUIDHelper = this.WhenAnyValue(x => x.UUID, x => x.CanAddToLoadOrder)
		...
		_hasToolkitIconHelper = this.WhenAnyValue(x => x.IsToolkitProject).CombineLatest(whenDisplayExtraIcons)
			.AllTrue()
			.ToUIPropertyImmediate(this, x => x.HasToolkitIcon);

		_hasLooseModIconHelper = this.WhenAnyValue(x => x.IsLooseMod, x => x.IsToolkitProject, (b1,b2) => b1 && !b2).CombineLatest(whenDisplayExtraIcons)
			.AllTrue()
			.ToUIPropertyImmediate(this, x => x.HasLooseModIcon);

		_hasOverrideIconHelper = this.WhenAnyValue(x => x.IsForceLoaded).CombineLatest(whenDisplayExtraIcons)
			.AllTrue()
		...
		_hasToolTipHelper = this.WhenAnyValue(x => x.Description, x => x.HasDependencies, x => x.UUID).
		...
		_canDeleteHelper = this.WhenAnyValue(x => x.IsLooseMod, x => x.IsHidden, x => x.FilePath,
			(isEditorMod, isHidden, path) => !isEditorMod && !isHidden && path.IsValid())
			.ToUIPropertyImmediate(this, x => x.CanDelete);

		_scriptExtenderSupportToolTipTextHelper = this.WhenAnyValue(x => x.ExtenderModStatus, x => x.CurrentExtenderVersion, 
		...
		_hasExtenderStatusHelper = this.WhenAnyValue(x => x.ExtenderModStatus).Select(x => x != ModExtenderStatus.None)
		...
		_extenderIconHelper = this.WhenAnyValue(x => x.ExtenderModStatus).Select(ExtenderModStatusToIcon).ToUIPropertyImmediate(this, x => x.ExtenderIcon);

		_notesHelper = this.WhenAnyValue(x => x.ModManagerConfig, x => x.ModManagerConfig!.Notes).Select(x => x.Item2 ?? string.Empty).ToUIProperty(this, x => x.Notes, "");
		_hasNotesHelper = this.WhenAnyValue(x => x.Notes).Select(Validators.IsValid).ToUIProperty(this, x => x.HasNotes);

		_lastModifiedDateTextHelper = this.WhenAnyValue(x => x.LastModified).SkipWhile(x => !x.HasValue)
		...
		_hasFilePathHelper = this.WhenAnyValue(x => x.FilePath).Select(Validators.IsValid).ToUIProperty(this, x => x.HasFilePath);
		_displayVersionHelper = this.WhenAnyValue(x => x.Version.Version).ToUIProperty(this, x => x.DisplayVersion, "0.0.0.0");
		...
		_forceAllowInLoadOrderLabelHelper = this.WhenAnyValue(x => x.ForceAllowInLoadOrder).Select(b => b ? Loca.Mod_Command_ForceAllowInLoadOrder_Disable : Loca.Mod_Command_ForceAllowInLoadOrder_Enable).ToUIProperty(this, x => x.ForceAllowInLoadOrderLabel);
